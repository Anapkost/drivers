...
static int prob(struct pci_dev *pdev, const struct pci_device_id *ent) {
    int result;
    dev_t dev_num;
    
    // Регистрируем символьный драйвер
    result = alloc_chrdev_region(&dev_num, 0, 1, DRIVER_NAME);
    if (result < 0) {
        printk(KERN_ERR "Failed to allocate chrdev region\n");
        return result;
    }
    
    // Создаем устройство символьного драйвера
    cdev_init(&r8169, &r8169_fops);
    r8169.owner = THIS_MODULE;
    
    // Добавляем устройство символьного драйвера в систему
    result = cdev_add(&r8169, dev_num, 1);
    if (result < 0) {
    ...
    // Устанавливаем значение major-номера
